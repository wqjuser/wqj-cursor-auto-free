...
def get_install_location_from_registry() -> Optional[str]:
    ...
    if platform.system() != "Windows":
        logging.debug("非 Windows 系统，跳过注册表查找")
        return None
        
    logging.debug("开始从注册表查找 Cursor 安装位置...")
    registry_paths = [
    ...
    for hkey, path in registry_paths:
        logging.debug(f"正在检查注册表路径: HKEY_CURRENT_USER\\{path}")
        try:
            with winreg.OpenKey(hkey, path) as key:
                logging.debug(f"成功打开注册表键: HKEY_CURRENT_USER\\{path}")
                try:
                    # 使用 DisplayIcon 值来获取 Cursor.exe 的路径
                    cursor_exe = winreg.QueryValueEx(key, "DisplayIcon")[0]
                    logging.debug(f"找到 DisplayIcon 值: {cursor_exe}")
                    
                    if cursor_exe:
                        logging.debug(f"DisplayIcon 值不为空")
                        if os.path.isfile(cursor_exe):
                            logging.info(f"从注册表找到 Cursor 安装位置: {cursor_exe}")
                            return cursor_exe
                        else:
                            logging.debug(f"DisplayIcon 指向的文件不存在: {cursor_exe}")
                    else:
                        logging.debug("DisplayIcon 值为空")
                except WindowsError as e:
                    logging.debug(f"无法读取 DisplayIcon 值: {str(e)}")
        except WindowsError as e:
            logging.debug(f"无法打开注册表键 HKEY_CURRENT_USER\\{path}: {str(e)}")
            continue
    
    logging.debug("在注册表中未找到 Cursor 安装位置")
    return None
        ...
        for proc in psutil.process_iter(['name', 'exe']):
            try:
                if proc.info['name'].lower() == 'cursor.exe':
                    ...
                    logging.info(f"从运行进程找到 Cursor: {exe_path}")
                    return exe_path
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue
    except Exception as e:
        logging.error(f"查找进程时出错: {str(e)}")
    return None
...
async def find_cursor_exe() -> Optional[str]:
    ...
    """
    logging.info("开始搜索 Cursor 可执行文件...")
    ...
    if system == "Windows":
        ...
        logging.info("检查环境变量定义的路径...")
        for base_path in env_paths:
            if base_path:
                cursor_path = os.path.join(base_path, "Cursor.exe")
                logging.debug(f"检查路径: {cursor_path}")
                if os.path.isfile(cursor_path):
                    logging.info(f"找到 Cursor.exe: {cursor_path}")
                    return cursor_path
        
        # 检查所有驱动器下的常见安装位置
        logging.info("检查所有驱动器下的安装位置...")
        common_install_dirs = [
            "Cursor",
            "cursor",
        ...
        for drive in get_drives():
            logging.debug(f"检查驱动器 {drive}")
            for install_dir in common_install_dirs:
                full_path = f"{drive}\\{install_dir}\\Cursor.exe"
                logging.debug(f"检查路径: {full_path}")
                if os.path.isfile(full_path):
                    logging.info(f"找到 Cursor.exe: {full_path}")
                    return full_path
        
        # 检查环境变量PATH中的所有目录
        if "PATH" in os.environ:
            logging.info("在 PATH 环境变量中搜索...")
            for path_dir in os.environ["PATH"].split(os.pathsep):
                location = os.path.join(path_dir, "Cursor.exe")
                logging.debug(f"检查 PATH 中的路径: {location}")
                if os.path.isfile(location):
                    logging.info(f"在 PATH 中找到 Cursor.exe: {location}")
                    return location
        
        # 如果在常见位置没找到，尝试使用where命令
        logging.info("使用 where 命令搜索 Cursor.exe...")
        try:
            import subprocess
            ...
            if result.returncode == 0:
                ...
                if paths:
                    logging.info(f"使用 where 命令找到 Cursor.exe: {paths[0]}")
                    return paths[0]  # 返回第一个找到的路径
        except Exception as e:
            logging.error(f"使用 where 命令搜索失败: {str(e)}")
        ...
        for path in common_paths:
            if os.path.isfile(path):
                logging.info(f"在 macOS 上找到 Cursor: {path}")
                return path
        ...
        for path in common_paths:
            if os.path.isfile(path):
                logging.info(f"在 Linux 上找到 Cursor: {path}")
                return path
    ...
    # 如果所有方法都失败了，提示用户输入路径
    logging.warning(f"未找到 Cursor 可执行文件，请手动输入完整路径")
    time.sleep(1)
    user_path = input("请输入Cursor可执行文件的完整路径: ").strip()
    if os.path.isfile(user_path):
        logging.info(f"使用用户提供的路径: {user_path}")
        return user_path
        
    logging.error("无法找到有效的Cursor可执行文件路径")
    return None
...
async def get_cursor_paths() -> Tuple[str, str]:
    ...
    if system == "Windows":
        ...
        if app_path:
            ...
            if os.path.exists(resources_path):
                ...
                logging.warning(f"resources/app 目录不存在: {resources_path}")
        
        # 如果找不到，使用默认路径
        logging.info("使用默认安装路径...")
    ...
    if system == "Linux":
        logging.info("在 Linux 系统中搜索 Cursor 安装路径...")
...
def check_system_requirements(pkg_path: str, main_path: str) -> bool:
    ...
    for file_path in [pkg_path, main_path]:
        ...
        if not os.access(file_path, os.W_OK):
            logging.error(f"没有文件写入权限: {file_path}")
            return False
...
def version_check(version: str, min_version: str = "", max_version: str = "") -> bool:
    ...
    try:
        if not re.match(version_pattern, version):
            logging.error(f"无效的版本号格式: {version}")
            return False
        ...
        if max_version and current > parse_version(max_version):
            logging.error(f"版本号 {version} 大于最大要求 {max_version}")
            return False
...
async def modify_main_js(main_path: str) -> bool:
    ...
    try:
        ...
        logging.info("main.js文件修改成功")
        return True

    except Exception as e:
        logging.error(f"修改文件时发生错误: {str(e)}")
...
def backup_files(pkg_path: str, main_path: str) -> bool:
    ...
    try:
        ...
        if os.path.exists(backup_main):
            logging.info(f"main.js已存在备份文件，跳过备份步骤")
            return True
            
        if os.path.exists(main_path):
            shutil.copy2(main_path, backup_main)
            logging.info(f"已备份 main.js: {backup_main}")
...
async def main(restore_mode=False) -> None:
    ...
    try:
        ...
        try:
            ...
            logging.error(f"无法读取版本号: {str(e)}")
            sys.exit(1)

        # 检查版本
        if not version_check(version, min_version="0.45.0"):
            logging.error("版本不符合要求（需 >= 0.45.x）")
            sys.exit(1)

        logging.info("版本检查通过，准备修改文件")

        # 备份文件
        if not backup_files(pkg_path, main_path):
            logging.error("文件备份失败，终止操作")
            sys.exit(1)
